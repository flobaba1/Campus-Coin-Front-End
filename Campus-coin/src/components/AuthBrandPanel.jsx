...
function AuthBrandPanel({ eyebrow = 'Student finance, simplified', title = 'Know where your allowance goes. Keep more of it.', description = 'Log allowance, gigs and scholarships, set simple budgets, and understand your spending without connecting a bank account.', compact = false }) {
  return (
    <aside className={`auth-brand-panel ${compact ? 'auth-brand-panel-compact' : ''}`}>
      ...
      <div className="auth-brand-benefits">
        <span><Icon name="check" size={15} /> No bank login needed</span>
        <span><Icon name="check" size={15} /> Private by default</span>
        <span><Icon name="check" size={15} /> Free for students</span>
      </div>
